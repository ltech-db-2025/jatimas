...
class IndexedListView extends StatefulWidget {
  ...
  const IndexedListView({this.items, this.itemHeight, this.itemBuilder, this.scrollController, super.key});
...
class _IndexedListViewState extends State<IndexedListView> {
  ...
  void _scrollToItems(String char, Map<String, int> alphabetDyPositions) {
    ...
    double dyToGo = double.tryParse(indexToGo.toString()) ?? 0;
    dyToGo = dyToGo * widget.itemHeight!;
      ...
      builder: (context, constraint) {
        ...
        return Container(
          ...
          child: GestureDetector(
            onVerticalDragUpdate: (DragUpdateDetails dragUpdateDetails) => _onVerticalDragUpdate(dragUpdateDetails, alphabets, alphabetDyPositions),
            onVerticalDragStart: (DragStartDetails dragStartDetails) => _onVerticalDragStart(dragStartDetails, alphabets, alphabetDyPositions),
            onVerticalDragEnd: _onVerticalDragEnd,
            child: Column(
              mainAxisAlignment: MainAxisAlignment.end,
              crossAxisAlignment: CrossAxisAlignment.end,
              children: [...List.generate(alphabets.length, (index) => _getAlphabetItem(alphabets[index]))],
            ),
          ),
        );
      },
