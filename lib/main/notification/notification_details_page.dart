...
class _NotificationDetailsPageState extends State<NotificationDetailsPage> {
  ...
  void initState() {
    super.initState();
    displayedDate = AwesomeDateUtils.parseDateToString(AwesomeDateUtils.utcToLocal(AwesomeDateUtils.parseStringToDate(widget.receivedNotification.displayedDate.toString())!), format: 'dd/MM/yyyy HH:mm');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      value: bigPicture == null ? SystemUiOverlayStyle.dark : SystemUiOverlayStyle.light,
      child: Stack(
        children: <Widget>[
          ListView(
                  ...
                  children: <Widget>[
                      ...
                      children: <Widget>[
                          ...
                          children: <Widget>[
                            bigPicture == null
                                ? Container(
                                    ...
                                    decoration: const BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Colors.black12, Colors.transparent], stops: [0.0, 1.0])),
                                  )
                                : SizedBox(
                                    ...
                                    child: ShaderMask(
                                        shaderCallback: (rect) {
                                          return const LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Colors.black, Colors.black, Colors.transparent], stops: [0.0, 0.75, 0.98]).createShader(Rect.fromLTRB(0, 0, rect.width, rect.height));
                                        },
                        ...
                        Container(
                          ...
                          child: RichText(
                            ...
                            TextSpan(text: widget.receivedNotification.titleWithoutHtml ?? ((widget.receivedNotification.body?.isEmpty ?? true) ? '' : widget.receivedNotification.bodyWithoutHtml), style: TextStyle(fontSize: (widget.receivedNotification.title?.isEmpty ?? true) ? 22 : 32, height: 1.2, color: Colors.black, fontWeight: FontWeight.bold)),
                            TextSpan(
                              text: '\n$displayedDate',
                              style: themeData.textTheme.headlineMedium?.copyWith(color: Colors.black26),
                            )
                          ])),
                        )
                      ],
                    ...
                    (widget.receivedNotification.title?.isEmpty ?? true)
                        ...
                        : (widget.receivedNotification.body?.isEmpty ?? true)
                            ? const SizedBox.shrink()
                            : Container(width: mediaQueryData.size.width, padding: const EdgeInsets.only(top: 10, left: 20, right: 20, bottom: 25), child: Text(widget.receivedNotification.bodyWithoutHtml ?? '', style: themeData.textTheme.bodyMedium)),
                  ],
                ),
              ),
              Container(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            Text(
                              ...
                              style: themeData.textTheme.titleLarge?.copyWith(color: themeData.hintColor),
                            ),
                            const SizedBox(height: 20),
                            Text(
                              widget.results,
                              style: themeData.textTheme.titleMedium?.copyWith(color: themeData.hintColor),
                            ),
                          ],
                        )),
                  ],
